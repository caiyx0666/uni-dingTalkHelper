	...
	<view class="content" v-if="showContent">
		<u-navbar :is-back="false" title-color="#ffffff" :background="{background: '#007aff'}" title="">
			<view class="nav-main">
				...
				<view class="nav-right" style="margin-bottom: -10rpx;">
					<u-icon name="lock" style="margin-right: 20rpx;font-size: 36rpx;color: white;"
						@click="checkShouldShowSecurity(true)"></u-icon>
				</view>
			</view>
		...
		<uni-list style="width: 100%;">
			...
			<uni-list-item title="就绪状态">
				<template slot="footer">
					<view
						:class="status==='已就绪'?'list-item-subtitle status-enable':'list-item-subtitle status-disable'">
						{{status}}
					</view>
				</template>
			</uni-list-item>
      ...
      <uni-list-item title="测试息屏自动唤醒" clickable @click="testRouse">
			...
			<uni-list-item title="【上班】打卡起始时间" clickable @click="gotoStartTimerPickerConfig.show=true">
				<template slot="footer">
					<view class="list-item-subtitle">
						{{gotoStartTimerPickerConfig.value.length?gotoStartTimerPickerConfig.value:'点击设置'}}
					</view>
				</template>
			</uni-list-item>
			...
			<uni-list-item title="【上班】打卡结束时间" clickable @click="gotoEndTimerPickerConfig.show=true">
				<template slot="footer">
					<view class="list-item-subtitle">
						{{gotoEndTimerPickerConfig.value.length?gotoEndTimerPickerConfig.value:'点击设置'}}
					</view>
				</template>
			</uni-list-item>
			...
			<uni-list-item title="【下班】打卡起始时间" clickable @click="gooffStartTimerPickerConfig.show=true">
				<template slot="footer">
					<view class="list-item-subtitle">
						{{gooffStartTimerPickerConfig.value.length?gooffStartTimerPickerConfig.value:'点击设置'}}
					</view>
				</template>
			</uni-list-item>
			<uni-list-item title="【下班】打卡结束时间" clickable @click="gooffEndTimerPickerConfig.show=true">
				<template slot="footer">
					<view class="list-item-subtitle">
						{{gooffEndTimerPickerConfig.value.length?gooffEndTimerPickerConfig.value:'点击设置'}}
					</view>
				</template>
			</uni-list-item>
		...
		<u-picker @confirm="e=>{timePickerConfirm(e,1)}" mode="time" :title="gotoStartTimerPickerConfig.title"
			:default-time="gotoStartTimerPickerConfig.value.length?gotoStartTimerPickerConfig.value:gotoStartTimerPickerConfig.default"
			:params="{'hour': true,'minute': true}" v-model="gotoStartTimerPickerConfig.show"></u-picker>
		<u-picker @confirm="e=>{timePickerConfirm(e,2)}" mode="time" :title="gotoEndTimerPickerConfig.title"
			:default-time="gotoEndTimerPickerConfig.value.length?gotoEndTimerPickerConfig.value:gotoEndTimerPickerConfig.default"
			:params="{'hour': true,'minute': true}" v-model="gotoEndTimerPickerConfig.show"></u-picker>
		<u-picker @confirm="e=>{timePickerConfirm(e,3)}" mode="time" :title="gooffStartTimerPickerConfig.title"
			:default-time="gooffStartTimerPickerConfig.value.length?gooffStartTimerPickerConfig.value:gooffStartTimerPickerConfig.default"
			:params="{'hour': true,'minute': true}" v-model="gooffStartTimerPickerConfig.show"></u-picker>
		<u-picker @confirm="e=>{timePickerConfirm(e,4)}" mode="time" :title="gooffEndTimerPickerConfig.title"
			:default-time="gooffEndTimerPickerConfig.value.length?gooffEndTimerPickerConfig.value:gooffEndTimerPickerConfig.default"
...
<script>
	...
	export default {
		...
		onShow() {
			this.screenStatus = 'show'
			this.updateGotoTime();
			this.updateGooffTime();
			this.pwdStatus = this.$storage.getSecurityPwd().length === 0 ? 0 : 1;
			pushMessageEmail({
				subject: '钉钉打卡助手-设置打卡时间',
		...
		methods: {
			...
			// 查看是否在随机访问内
			checkRandomRange(time) {
				// 是否今天要打卡
				if(!this.selectedWeeksArr.includes(time.getDay())) return
				const date = new Date()
				const curTimeStr = `${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()}`
				const startTime = new Date(curTimeStr + ' ' + this.gotoStartTimerPickerConfig.value).getTime()
				const endTime = new Date(curTimeStr + ' ' + this.gotoEndTimerPickerConfig.value).getTime()
				if(
			...
			timePickerConfirm(e, type) {
				...
				if (type === 1) {
					if (this.gotoEndTimerPickerConfig.value && this.$utils.getMinAndSecondDiff(this
							.gotoEndTimerPickerConfig.value,
							time) >= 0) {
						this.$refs.uTips.show({
					...
					if (this.gotoStartTimerPickerConfig.value && this.$utils.getMinAndSecondDiff(this
							.gotoStartTimerPickerConfig.value,
							time) <= 0) {
						this.$refs.uTips.show({
					...
					if (this.gooffEndTimerPickerConfig.value && this.$utils.getMinAndSecondDiff(this
							.gooffEndTimerPickerConfig.value,
							time) >= 0) {
						this.$refs.uTips.show({
					...
					if (this.gooffStartTimerPickerConfig.value && this.$utils.getMinAndSecondDiff(this
							.gooffStartTimerPickerConfig.value,
							time) <= 0) {
						this.$refs.uTips.show({
			...
			getTimePickerConfig(index) {
				const configs = [this.gotoStartTimerPickerConfig, this.gotoEndTimerPickerConfig, this
					.gooffStartTimerPickerConfig,
			...
			updateGotoTime() {
				console.log('updateGotoTime-------------------')
				this.gotoTime = this.getGotoTime();
			},
			updateGooffTime() {
				console.log('updateGooffTime-------------------')
			...
			getGotoTime() {
				...
				this.startTimer();
				return this.$utils.getNextClockFullTime(this.gotoStartTimerPickerConfig.value, this
					.gotoEndTimerPickerConfig.value,
			...
			getGooffTime() {
				...
				this.startTimer();
				return this.$utils.getNextClockFullTime(this.gooffStartTimerPickerConfig.value, this
					.gooffEndTimerPickerConfig.value,
					this.selectedWeeksArr);
			},
			openDingtalkClick() {
